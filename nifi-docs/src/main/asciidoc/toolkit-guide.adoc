 ...
 * `-S`,`--protectionScheme <protectionScheme>`  Selects the protection scheme for encrypted properties.  Valid values are: [AES_GCM, HASHICORP_VAULT_TRANSIT, GCP_KMS] (default is AES_GCM)
...
|`gcp.kms.keyring`|The keyring containing the key that the GCP KMS client uses for encryption and decryption.|_none_
|`gcp.kms.key`|The key identifier that the GCP KMS client uses for encryption and decryption.|_none_
|===
